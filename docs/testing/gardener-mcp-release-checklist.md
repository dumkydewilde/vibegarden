...
| Artifact kinds | `create_artifact` with `type: "link"` and an https `url` stores a link whose card opens that address; `files` plus `url`, an `http` url, and another club's project are all refused; `create_artifact_version` with a `url` repoints the link and is refused for an HTML package. | `PENDING` |
