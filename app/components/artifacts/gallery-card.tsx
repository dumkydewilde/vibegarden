...
export function GalleryCard({ artifact }: { artifact: GalleryCardData }) {
  ...
  const className = "block rounded-lg border bg-card p-4 transition-colors hover:border-primary/40 hover:bg-accent/30";
  return artifact.type === "link" && artifact.externalUrl
    ? <a href={artifact.externalUrl} target="_blank" rel="noopener noreferrer" className={className}>{contents}</a>
    : <Link to={artifact.url} className={className}>{contents}</Link>;
}
