...
```
my-project/
  README.md          <- what this is, and a map of the rest
  people.md          <- who is who, and who decides what
  definitions.md     <- what words mean here
  decisions.md       <- choices made, and why
  processes/
    onboarding.md    <- how a new member joins
...
Once your knowledge lives in files, a model needs hands to reach them: [What is an agent?](/learning/what-is-an-agent) explains the tool loop that does the reading. And if you are still tempted to solve a quality problem with a pricier model, reread [Choosing a model](/learning/choosing-a-model) first: upgrade the briefing before the intern. For the version of all this that happens while you build, [Vibe coding: how to work with a coding agent](/learning/working-with-a-coding-agent) puts the briefing habit to work one step at a time.
