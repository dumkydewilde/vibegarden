...
export function buildReviewerSeedSql(rawEmail) {
  ...
  const statements = [
    ...
    `INSERT INTO club_memberships (club_id, user_id, role, onboarding_stage, joined_at, updated_at) VALUES (${literal(clubId)}, ${literal(reviewer)}, 'member', 'exploring', ${createdAt}, ${createdAt}) ON CONFLICT(club_id, user_id) DO NOTHING;`,
    `UPDATE users SET last_club_id=${literal(clubId)} WHERE id=${literal(reviewer)};`,
    `INSERT INTO chat_threads (id, user_id, club_id, title, project_id, created_at, updated_at) VALUES (${literal(threadOne)}, ${literal(reviewer)}, ${literal(clubId)}, ${literal("Plan a neighbourhood herb garden")}, NULL, ${createdAt}, ${createdAt + 7}) ON CONFLICT(id) DO UPDATE SET club_id=excluded.club_id, title=excluded.title, project_id=NULL, updated_at=excluded.updated_at;`,
    `INSERT INTO chat_threads (id, user_id, club_id, title, project_id, created_at, updated_at) VALUES (${literal(threadTwo)}, ${literal(reviewer)}, ${literal(clubId)}, ${literal("Share the harvest")}, NULL, ${createdAt + 8}, ${createdAt + 15}) ON CONFLICT(id) DO UPDATE SET club_id=excluded.club_id, title=excluded.title, project_id=NULL, updated_at=excluded.updated_at;`,
    `INSERT INTO projects (id, user_id, club_id, title, one_liner, modules, status, thread_id, created_at, updated_at) VALUES (${literal(reviewerId(email, "project-seed"))}, ${literal(reviewer)}, ${literal(clubId)}, ${literal("Herb garden sketch")}, ${literal("A small shared planter for neighbours.")}, ${literal('["web-app"]')}, 'seed', ${literal(threadOne)}, ${createdAt}, ${createdAt}) ON CONFLICT(id) DO UPDATE SET club_id=excluded.club_id, title=excluded.title, one_liner=excluded.one_liner, modules=excluded.modules, status='seed', thread_id=excluded.thread_id, updated_at=excluded.updated_at;`,
    `INSERT INTO projects (id, user_id, club_id, title, one_liner, modules, status, thread_id, created_at, updated_at) VALUES (${literal(reviewerId(email, "project-growing"))}, ${literal(reviewer)}, ${literal(clubId)}, ${literal("Watering rota")}, ${literal("A friendly schedule for garden care.")}, ${literal('["scheduled-task","google-sheet"]')}, 'growing', ${literal(threadTwo)}, ${createdAt + 8}, ${createdAt + 8}) ON CONFLICT(id) DO UPDATE SET club_id=excluded.club_id, title=excluded.title, one_liner=excluded.one_liner, modules=excluded.modules, status='growing', thread_id=excluded.thread_id, updated_at=excluded.updated_at;`,
    `INSERT INTO projects (id, user_id, club_id, title, one_liner, modules, status, thread_id, created_at, updated_at) VALUES (${literal(reviewerId(email, "project-bloomed"))}, ${literal(reviewer)}, ${literal(clubId)}, ${literal("Harvest sharing board")}, ${literal("A simple way to offer extra herbs.")}, ${literal('["web-app","database"]')}, 'bloomed', NULL, ${createdAt + 16}, ${createdAt + 16}) ON CONFLICT(id) DO UPDATE SET club_id=excluded.club_id, title=excluded.title, one_liner=excluded.one_liner, modules=excluded.modules, status='bloomed', thread_id=NULL, updated_at=excluded.updated_at;`,
  ];
